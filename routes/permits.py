...
import models
from email_service import send_application_submitted_email, send_application_submitted_admin_email
...
def apply():
    ...
    if request.method == 'POST':
        ...
        # Get user and permit type info for email
        cur = models.get_db(mysql)
        user = models.get_user_by_id(cur, session['user_id'])
        ...
        send_application_submitted_email(
            ...
            project_title=project_title,
            submitted_date=request.form.get('submitted_date', ''),  # Will be generated by DB
            is_hazard=is_hazard,
            hazard_info=hazard_info
        )

        # Send email to admin
        review_link = f"{request.host_url}admin/application/{app_id}/review"
        send_application_submitted_admin_email(
            app_id=app_id,
            user_name=user['full_name'],
            user_email=user['email'],
            permit_type=permit_type['name'],
        ...
        if is_hazard:
            flash(f'Application submitted. ⚠️ Warning: selected location is in a hazard zone ({hazard_info}). Check your email for confirmation.', 'warning')
        else:
